...
def api_upload():
    ...
    for f in request.files.getlist("files"):
        ...
        try:
            ...
            f.save(out_path)
            thumb = thumb_b64(out_path, 100)
            saved.append({"name": os.path.basename(out_path), "thumb": thumb})
        except Exception as e:
...
/* discord banner */
.discord-banner{display:flex;align-items:center;gap:10px;padding:9px 12px;
...
<aside class="sidebar-r">
  ...
  <a href="https://discord.com/invite/loras" target="_blank" class="discord-banner">
    ...
    <div class="discord-text">
      <div class="discord-title">Join our Community</div>
      <div class="discord-sub">LoRAs, workflows &amp; more</div>
    </div>
...
function uploadFiles(dsName, files, onDone){
  ...
  files.forEach(f=>{
    var el=document.getElementById('ust-'+safeId(f.name));
    if(el){el.textContent='uploading';el.className='ust pend';}
  });
  var BATCH=10, done=0, errs=0;
  function batch(i){
    if(i>=files.length){
      if(errs>0)setStatus('Upload done: '+done+' saved, '+errs+' failed — check console for details.');
    ...
    fetch('/api/upload',{method:'POST',body:fd})
      .then(r=>{
        if(!r.ok)throw new Error('Upload failed: server returned '+r.status);
        return r.json();
      })
      .then(res=>{
        (res.saved||[]).forEach(s=>{
          done++;
          // Try to find by original name or modified name
          var origFile=b.find(f=>f.name===s.name||safeId(f.name)===safeId(s.name));
          var sid=origFile?safeId(origFile.name):safeId(s.name);
          var el=document.getElementById('ust-'+sid);
          if(el){el.textContent='✓';el.className='ust ok';}
        });
        (res.errors||[]).forEach(e_=>{
          errs++;
          console.error('Upload error:',e_);
        });
        batch(i+BATCH);
      })
      .catch(err=>{
        errs+=b.length;
        b.forEach(f=>{var el=document.getElementById('ust-'+safeId(f.name));if(el){el.textContent='✗';el.className='ust err';}});
